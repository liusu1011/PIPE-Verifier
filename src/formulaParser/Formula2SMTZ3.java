...
//	ArrayList<String> post_conds;
	ArrayList<String> extra_vars;//iniclude declarations of uservariables and temp variables
	int stateId;
...
//		String z3var = "S"+stateId+"_"+placeName+"_"+userVariable;
	...
	public void visit(ConstantTerm elem) {
		...
		if(elem.c instanceof NumConstant){
			...
			elem.z3str = "mk_int(ctx, "+elem.int_val+")";
...
//		String var_key = "";
		...
		if(elem.v instanceof IdVariable){
			...
			while(itr_out.hasNext()){
				...
				if(thisArc.getVar().equals(elem.var_key)){
					//its target is a place, source is a transition
					elem.isPostCond = true;
					placeName = thisArc.getTarget().getName();	
					break;
				}
			}
			Iterator<Arc> itr_in = iTransition.getArcInList().iterator();
			while(itr_in.hasNext()){
				Arc thisArc = itr_in.next();
				if(thisArc.getVar().equals(elem.var_key)){
					//its target is a transition, source is a place
			...
			if(placeName.equals(""))
				System.out.println("VariableTerm: Can't find related placeName!");
			...
			if(elem.index<1)System.out.println("Error: variable index cannot less than 1");
